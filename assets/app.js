...
document.addEventListener('DOMContentLoaded', function() {
    let $giftCheckbox = (0, _lib.$)('#gift-check');
    let $giftForm = (0, _lib.$)('.gift-card-recipient-form');
    const emailInput = (0, _lib.$)('#recipient-email');
    const nameInput = (0, _lib.$)('#recipient-name');
    const messageInput = (0, _lib.$)('#recipient-message');
    if ($giftCheckbox) $giftCheckbox.addEventListener('change', function() {
    ...
    fetch(`/collections/${handle}/products.json`).then((response)=>response.json()).then((data)=>{
        setTimeout(()=>{
            ...
            data.products.forEach((product)=>{
                ...
                if (product.variants[0].compare_at_price > product.variants[0].price) is_on_sale = true;
...
------------------------------------------------------------------*/ document.addEventListener('DOMContentLoaded', ()=>{
    ...
    let $thumbnailSlider = (0, _lib.$)('#thumbnail-slider');
    if ($productSlider) {
        ...
        if ($thumbnailSlider) var thumbnails = new (0, _splideDefault.default)('#thumbnail-slider', {
            direction: 'ttb',
            heightRatio: 1,
            fixedWidth: 80,
            fixedHeight: 80,
        ...
        if ($thumbnailSlider) main.sync(thumbnails);
        main.mount();
        if ($thumbnailSlider) thumbnails.mount();
        let images = [];
